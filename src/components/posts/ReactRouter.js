...
function ReactPost() {
    return (
        <div className="container post-content">
            ...
            <p>First things first, let's install react-router-dom.  This library works for web-based React applications, and is hosted on the public npm registry.  That makes things pretty easy!  From your command line, run <code>npm install react-router-dom</code>.  This will install the necessary tools to use react-router.</p>
...
`}</code></pre>
            ...
            <p>Let's suppose for this blog that we want to use the standard index generated by create-react-app as our home page.  Underneath the <code>&lt;div&gt; className="App"&lt;div&gt;</code> line, insert a Router and enclosed Switch tag, and finally a Route tag with the path attribute set to "/", and enclose the rest of the code that was in the division tag as children of the Route tag, so that your file structure looks like this:</p>
            <pre><code>
{`function App() {
  return (
    <div className="App">
...
`}</code></pre>
        <p>This sets the content within the Route tag to map to the "/" or index path.  Run <code>npm start</code> to start the development server, and navigate to <code>http://localhost:3000</code>, and you should see the default React page.</p>
        <img src={reactdefaultImage} alt="react default page" className="img-fluid"/>
        <p>This means that react-router is successfully mapping our index page to the react default page.  If you try putting in a different path in the address bar, for example <code>http://localhost:3000/foo/bar</code>, you'll notice that the index page is still served.  We can change this configuration later if you would prefer to throw a 404 response, but this can also be a useful configuration if you would prefer to direct users to your home page.</p>
        <hr />
        <h2>Configuring additional routes</h2>
        <p>Let's add a second route to our configuration.  An important thing to note with react-router is that when we're configuring routes, we need to start at the most specific and then work our way down to the least specific.  For example, we would want to order the routes <code>/</code>, <code>/foo</code>, and <code>/foo/bar</code> like this:</p>
        <ol>
            <li><code>/foo/bar</code></li>
            <li><code>/foo</code></li>
            <li><code>/</code></li>
        </ol>
        <p>So let's say that we wanted to configure a route for <code>/test</code> to serve just a header that says test.  Add another <code>&lt;Route&gt;</code> tag with a path attribute <code>/test</code>, and the header tag as a child element, like so:</p>
        <pre><code>
{`function App() {
  return (
    <div className="App">
...
`}            
        ...
        <p>Now when you navigate to <code>http://localhost:3000/test</code>, you should see "test" displayed as a header.  Notice that if you try navigating to <code>http://localhost:3000/test/foo/bar</code>, the test page is still served.</p>
        <hr />
        <h2>react-router Links</h2>
        <p>react-router comes with a Link component, which can be used for navigating between pages.  Whereas in a simple HTML site you might navigate between pages using <code>&lt;a&gt;</code> tags, this won't work with the way that react-router maintains state.  Let's try replacing the link at the bottom of the default react page with a react-router link to the test page we just created.  In the Route component for the "/" route, replace the <code>&lt;a&gt;</code> tag with a Link tag, and set the "to" attribute to "/test", like so (the style attribute is included just to make it show up better on the page):</p>
        <pre><code>
{`function App() {
  return (
    <div className="App">
...
{`import {
  ...
  Route,
        ...
        <p>Now, even if you're using a static file store to serve the application, users can be deep linked to a page on your app.  Try typing <code>http://localhost:3000/#/test</code> into your address bar, to verify that it's working.</p>
