...
export default function ChaosGamePost() {
    return (
        <div className="post-content">
            ...
            <p className="post-body">The image that you see on this page is called a <code className="inline">Sierpinski Triangle</code>.  It's an example of a fractal, or an image that appears the same at any scale.  Fractals are <code className="inline">self-similar</code>, meaning that fractions of the image are similar to the image itself.  For example, if you zoom in on the Sierpinski Triangle so that only the top sub-triangle is visible, the image would look identical to the original Sierpinski Triangle.</p>
            <p>Let's take a brief aside and talk about <code className="inline">chaos theory</code>.  Chaos theory is the mathematical study of <i>chaos</i>, or systems that seem random and disordered, but actually obey mathematical laws.  When I think of chaos, I think of the vastness and apparent randomness of the physical universe.  While we may exist in a mess of atoms, all bouncing around randomly, in reality these particles all obey the laws of physics.  They aren't really bouncing around randomly, they actually transfer energy between each other and change properties in predictable ways.  </p>
            <p>An important principle in chaos theory is that the initial state of a deterministic system (a system where the same initial state will always produce the same output, but the change in output is not proportional to the change in input) can produce large changes in later states<sup><Link to="#footnote-1">[1]</Link></sup>.  This is often illustrated via the metaphor of the <code className="inline">butterfly effect</code>.  This name comes from a 1961 computational model created by Edward Lorenz to predict the weather<sup><Link to="#footnote-1">[1]</Link></sup>.  A simple rounding error in the starting state of his model (a difference on the order of one thousandth, roughly the difference that the flap of a butterfly's wings could make on a gentle breeze) led to vastly different outcomes.  Metaphorically, the flap of a butterfly's wings on one side of the globe could create a hurricane on the other side.  This is the core tenant of chaotic systems:  by changing initial conditions ever so slightly, huge differences in final state can occur.  When thinking about the universe, I think about the big bang, which set all of the particles that we see today into motion.  Studies suggest that tiny changes in the way the universe developed in it's first 10<sup>-43</sup> seconds could have completely changed the way it developed<sup><Link to="#footnote-2">[2]</Link></sup>.  If the starting conditions weren't precisely the way they occurred in our universe, there almost certainly wouldn't be butterflies to flap their wings, or humans to ponder the consequences, at least not the way we know them.</p>
            <h2>Pure Chaos</h2>
            <p>To demonstrate chaos, I wrote a program that simulates an experiment, called the <code className="inline">chaos game</code>.  To play the simulation, clone the repository linked at the top of the article.  The chaos game is pretty simple, and relies on one of the most fundamental aspects of chaos, it's apparent randomness.  The way it is played is by starting out with a geometric shape, with a number of vertices, and a <i>seed</i>, or a point on the same plane as the shape.  Each round of the game, one of the vertices is chosen at random, and the seed is moved half the distance toward the vertex.  On the next turn, another vertex is selected at random, the seed moves from it's new position half-way to the next, and the game continues in this fashion.  To start, let's try the chaos game using a square.  If you've cloned the repository, from the root directory run <code className="inline">python3 random_squares.py</code>.  A window should open, with the vertices of the square plotted and the seed placed inside.  Play around with the "Plot points" buttons, and see if you notice a pattern forming.</p>
            ...
            <h2>Attractors</h2>
            <p>If you're a chaos theorist, this game isn't very helpful.  Given the starting seed, no mathematician could tell you what points were going to be plotted.  There's no way to predict any outcome from any input.  So let's try changing the rules a bit.  Instead of randomly choosing one of the four corners to move towards, let's add one stipulation:  the next corner chosen can't be the same as the one you just moved towards.  We don't really have any reason to think this will change the random outcome, after all, we just showed that choosing points at random does create a random distribution.  Why would removing one choice make a difference?  Well, let's simulate it and see.  Try running <code className="inline">python3 squares.py</code>, and see if you can find a pattern.</p>
            <div className="post-img-left">
                <figure className="figure">
                    <img src={chaosSquares} alt="ordered square plot" className="img-fluid" />
                    <figcaption className="figure-caption">Chaos game simulation on a square, where the most recently chosen vertex cannot be chosen again, showing 500 iterations</figcaption>
                </figure>
            </div>
            <p className="post-body">The image to the left shows 500 iterations using this pattern.  You may notice that a pattern appears to be emerging.  There appear to be forbidden regions in the shape of squares, where points aren't plotted.  If you look closely, you may notice that the pattern generated is actually a complex fractal.  There is a pattern that repeats itself at smaller scales.  Now try moving the starting position around the plot.  You'll notice that changing the starting point changes the location of the first several points plotted.  However, when you plot a few hundred points, the fractal pattern emerges regardless.  This pattern is the <code className="inline">attractor</code> of the chaotic system, because after a sufficiently large number of iterations, the seemingly random system evolves into this state.  Think of the fractal grid as a gravitational field, pulling points closer and closer to it as the movement of the seed continues to be plotted.  After a certain number of iterations, the seed becomes gravitationally bound to the grid, and won't move away from it again.</p>
